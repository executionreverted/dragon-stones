...
contract DragonStonePieces is Initializable, OFTV2Upgradable, Proxied {
    ...
    function mintPiece(address to, uint amount) external onlyDragonContract {
        _mint(to, amount);
    }

    function burnPiece(address to, uint amount) external onlyDragonContract {
        _burn(to, amount);
    }
}
