...
contract DragonStoneGold is Initializable, OFTV2Upgradable, Proxied {
    ...
    function mintGold(address to, uint amount) external onlyDragonContract {
        _mint(to, amount);
    }

    function burnGold(address to, uint amount) external onlyDragonContract {
        _burn(to, amount);
    }
}
